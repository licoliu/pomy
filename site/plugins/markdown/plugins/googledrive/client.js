var GoogleDrive = (function() {
  ...
  function renderSearchResults(a, b, res) {
    ...
    if (!Array.isArray(result.items)) return _errorHandler(null, null, {
      responseText: "No Markdown files found!"
    })

    result.items.forEach(function(item) {
      list += '<li data-file-id="' + item.id + '"><a class="googledrive_file" href="#">' + item.title + '</a></li>'
    })
  ...
  return {
    ...
    save: function() {
      ...
      var postData = 'title=' + encodeURIComponent(profile.current_filename) + (hasExtension ? '' : editorType().fileExts[0]) + '&content=' + content
